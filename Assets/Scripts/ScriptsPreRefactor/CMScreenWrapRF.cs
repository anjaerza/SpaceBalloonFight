...
public class CMScreenWrapRF : MonoBehaviour
{	
    ...
    bool setOrtoSize = false;

    GameObject ghostLeft, ghostRight;

    private void Start()
    {
        // Invoke is necesary to catch correct OrtoSize from Cinemachine
        Invoke("PostStart", 1f);
        // PostStart();
    }
    ...
    private void PostStart()
	{
		...
		var cam = Camera.main; //hacer camara modificable aqui...
        ...
        //var screenBottomLeft = cam.ScreenToWorldPoint(new Vector3(0, 0, transform.position.z));
        //var screenTopRight = cam.ScreenToWorldPoint(new Vector3(Screen.height, Screen.width, transform.position.z));

        //Vector3 screenBottomLeft = new Vector3(-8.5f, -11.6f, -14.3f);
        //Vector3 screenTopRight = new Vector3(9.3f, 24.4f, -0.2f);
		...
		{	
            ...
            // Identify each ghost
            // ghostLeft = ghosts[0].gameObject;
            // ghostRight = ghosts[1].gameObject;
        }
	...
	void PostUpdate()
	{
            ...
            if (advancedWrapping)
            {
                ...
                //Update ghosts position by code cuz the ghosts dont have a movement script
                // if (ghostLeft != null && ghostRight != null)
                // {
                //     ghostLeft.transform.position = new Vector3(transform.position.x - screenWidth, transform.position.y, transform.position.z);
                //     ghostRight.transform.position = new Vector3(transform.position.x + screenWidth, transform.position.y, transform.position.z);
                // }
            }
            else
            {
	...
	void CreateGhostShips()
	{
		for(int i = 0; i < 2; i++)
		{
            ...
            //Add script to update renderers of our ghosts
            //ghosts[i].gameObject.AddComponent<UpdateGhostRenderers>();
            ghosts[i].gameObject.tag = "Ghost";

            // But without the screen wrapping component
            //DestroyImmediate(ghosts[i].GetComponent<CMScreenWrapRF>());
            //DestroyImmediate(ghosts[i].GetComponent<TouchMovementRF>());
            //DestroyImmediate(ghosts[i].GetComponent<Rigidbody>());
            //DestroyImmediate(ghosts[i].GetComponentInChildren<SpriteRenderer>());

            Destroy(ghosts[i].GetComponent<CMScreenWrapRF>());
            Destroy(ghosts[i].GetComponent<TouchMovement>());
         	Destroy(ghosts[i].GetComponent<Rigidbody>());
            Destroy(ghosts[i].GetComponent<InstantDamage>());
            Destroy(ghosts[i].GetComponent<CapsuleCollider>());
           	Destroy(ghosts[i].GetComponent<AudioSource>());

			ghosts[i].gameObject.AddComponent<GhostMovement>();
	...
	void PositionGhostShips()
	{
		...
		ghosts[0].gameObject.GetComponent<GhostMovement>().Side="Right"; 

        // // Bottom-right
        // ghostPosition.x = transform.position.x + screenWidth;
        // ghostPosition.y = transform.position.y - screenHeight;
        // ghosts[1].position = ghostPosition;
		// ghosts[1].gameObject.GetComponent<GhostMovement>().Side="DownRigh"; 
		...
		// ghosts[2].gameObject.GetComponent<GhostMovement>().Side="Down"; 
		...
		// ghosts[3].gameObject.GetComponent<GhostMovement>().Side="DownLeft"; 
		...
		ghostPosition.y = transform.position.y;
		ghosts[1].position = ghostPosition;
		ghosts[1].gameObject.GetComponent<GhostMovement>().Side="Left"; 
		...
		// ghosts[5].gameObject.GetComponent<GhostMovement>().Side="UPLeft"; 
		...
		// ghosts[6].gameObject.GetComponent<GhostMovement>().Side="Up"; 
		...
		// ghosts[7].gameObject.GetComponent<GhostMovement>().Side="UpRig"; 
    ...
    {
        GameObject[] lastGhosts = GameObject.FindGameObjectsWithTag("Ghost");

        foreach (GameObject go in lastGhosts)
        {
            Destroy(go);
        }
