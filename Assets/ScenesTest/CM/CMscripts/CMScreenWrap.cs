...
public class CMScreenWrap : MonoBehaviour
{	
    ...
    void NexStart()
	{
        ...
        //var screenBottomLeft = cam.ViewportToWorldPoint(new Vector3(0, 0, transform.position.z));
        //var screenTopRight = cam.ViewportToWorldPoint(new Vector3(Camera, 1, transform.position.z));
        //var screenBottomLeft = cam.ScreenToWorldPoint(new Vector2(0f, 0f));
        //var screenTopRight = cam.ScreenToWorldPoint(new Vector2(Screen.width, Screen.height));
        ...
        //screenWidth = cam.ScreenToWorldPoint(new Vector2(Screen.width, 0f)).x;
        //screenHeight = cam.ScreenToWorldPoint(new Vector2(Screen.width, Screen.height)).y;
	...
	void Update()
	{
        if (setOrtoSize)
        {
            if (advancedWrapping)
            {
                ...
                //Should called if renderes change by event or action
                renderers = GetComponentsInChildren<Renderer>();
            }
            else
            {
	...
	void CreateGhostShips()
	{
		for(int i = 0; i < 2; i++)
		{
			...
			DestroyImmediate(ghosts[i].GetComponent<CMScreenWrap>());
            //DestroyImmediate(ghosts[i].GetComponentInChildren<SpriteRenderer>());
            //DestroyImmediate(ghosts[i].GetChild(1).gameObject);
            //DestroyImmediate(ghosts[i].gameObject.GetComponent<Rigidbody>());
            //GetComponent<Rigidbody>().isKinematic = true;
            //GetComponent<Rigidbody>().useGravity = false;
            //ghosts[i].gameObject.GetComponent<Rigidbody>().useGravity = false;

            //DestroyImmediate(ghosts[i].gameObject.GetComponent<TouchMovement>());
            //DestroyImmediate(ghosts[i].gameObject.GetComponent<Rigidbody>());

            //ghosts[i].gameObject.GetComponent<TouchMovement>().enabled = false;
            //GetComponent<TouchMovementTwo>().enabled = true;

            //ghosts[i].gameObject.GetComponent<Rigidbody>().useGravity = false;

            //ghosts[i].gameObject.GetComponent<TouchMovementTwo>().enabled = true;
            //DestroyImmediate(ghosts[i].GetComponent<TouchMovement>());

            DestroyImmediate(ghosts[i].GetComponentInChildren<SpriteRenderer>());
            //print(ghosts[i].gameObject.name);
            //ghostLeft = ghosts
            DestroyImmediate(ghosts[i].GetComponent<TouchMovement>());
